...
setwd('d:/Projects/UCPH/Work/atacseq/new')

peak <- 'd:/Projects/UCPH/Work/atacseq/new/'
scales <- 'd:/Projects/UCPH/Work/atacseq/new/scale_factor/'
```


```{r}
count <- read.table(paste0(peak, 'raw_count_new.txt')) %>%
...
ct_vs_12_3_delete_design <- data.frame(ct_vs_12_design[-3])
rownames(ct_vs_12_3_delete_design) <- c('si673_12h_1','si673_12h_2','siNT_1', 'siNT_2', 'siNT_3')
colnames(ct_vs_12_3_delete_design) <- 'condition'
...
distheatmap_12
ggsave(distheatmap_12, file = 'sample_dist_heatmap_12h.png')
...
pca_2_12 <- ggplot(pca.x_12, aes(x = PC1, y = PC2, color = condition, 
                           label = condition)) +
  geom_point() + 
  xlab(label = paste('PC1', pca.var_12[1])) +
  ylab(label = paste('PC2', pca.var_12[2])) +
  geom_label_repel() +
  theme_bw()

pca_2_12

ggsave(pca_2_12, file = 'pca_plot_12h.png')
```
...
adjheatmap_12
ggsave(adjheatmap_12, file = 'gene_significant_heatmap_12hd.png')
...
write_csv(result_tb_24d, file = 'result_atac_24d.csv')
```
...
write_csv(result_padj_24d, file = 'high_padj_atac_24d.csv')
...
ggsave(volcano_24d, file = 'volcano_plot_atac_24d.png')
```
...
distheatmap_24d
ggsave(distheatmap_24d, file = 'sample_dist_heatmap_24hd.png')
...
ggsave(pca_2_24d, file = 'pca_plot_24hd.png')
```
...
adjheatmap_24d
ggsave(adjheatmap_24d, file = 'gene_significant_heatmap_24hd.png')
...
write_csv(result_tb_12d, file = 'result_atac_12d.csv')
```
...
write_csv(result_padj_12d, file = 'high_padj_atac_12d.csv')
...
ggsave(volcano_12d, file = 'volcano_plot_atac_12d.png')
```
...
distheatmap_12d
ggsave(distheatmap_12d, file = 'sample_dist_heatmap_12hd.png')
...
pca_2_12d <- ggplot(pca.x_12d, aes(x = PC1, y = PC2, color = condition, 
                           label = condition)) +
  geom_point() + 
  xlab(label = paste('PC1', pca.var_12d[1])) +
  ylab(label = paste('PC2', pca.var_12d[2])) +
  geom_label_repel() +
  theme_bw()

pca_2_12d

ggsave(pca_2_12d, file = 'pca_plot_12hd.png')
```
...
adjheatmap_12d <- pheatmap(mat_12d, 
                           ...
                           cellheight = 10)
adjheatmap_12d
ggsave(adjheatmap_12d, file = 'gene_significant_heatmap_12hd.png')
