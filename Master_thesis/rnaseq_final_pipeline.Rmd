...
```{r}
# DESeq2
design <- data.frame(condition = c('siNT(Ctrl)', 'siNT(Ctrl)', 'si673', 'si673',
                                   'si673_2', 'si673_2', 'siNT(Ctrl)', 'si673', 
                                   'si673_2'))

count <- read.table('d:/Projects/UCPH/Work/master_thesis/datasets/rnaseq_raw_count.txt')

colnames(count) <- c('siNT_1', 'siNT_2', 'si673_1', 'si673_2', 'si673_2_1', 'si673_2_2', 'siNT_3', 'si673_3', 'si673_2_3')
...
result <- results(deseq, lfcThreshold = 0.25, alpha = 0.05, 
                  contrast = c('condition', 'si673', 'siNT(Ctrl)'))
...
distheatmap <- pheatmap(sampleDistMatrix, 
                        ...
                        annotation_legend = F)
distheatmap
ggsave(distheatmap, file = paste0(figures, 'rnaseq_before_sample_dist_heatmap.png'))
...
samplecorheatmap
ggsave(samplecorheatmap, file = paste0(figures, 'rnaseq_before_sample_correlation_heatmap.png'))
...
ggsave(pca_2, file = paste0(figures, 'rnaseq_before_pca_plot.png'))
```
...
adjheatmap <- pheatmap(mat, annotation_col=design, 
                       ...
                       annotation_legend = F)
adjheatmap
ggsave(adjheatmap, file = paste0(figures, 'rnaseq_before_gene_significant_heatmap.png'))
...
count2 <- count %>%
  as.data.frame() %>%
  dplyr::select(-c('si673_2_1', 'si673_2_2', 'si673_2_3'))
...
write_csv(result_tb2, file = paste0(dataset, 'rnaseq_deseq_final_result_total.csv'))
...
write_csv(result_padj, file = paste0(dataset, 'rnaseq_deseq_final_high_padj.csv'))


subset <- result_padj %>%
  filter(symbol == 'LINC00511' | symbol == 'MET' | symbol == 'CDH1')

subset[subset$symbol =='LINC00511', 'symbol'] = 'LINC00673'
...
ggsave(volcano, file = paste0(figure, 'rnaseq_volcano_plot.png'))
```
...
pca_22 <- ggplot(pca.x2, aes(x = PC1, y = PC2, color = design2$condition, 
                           label = condition)) +
  geom_point() + 
  xlab(label = paste('PC1', 'with variance', pca.var2[1])) +
  ylab(label = paste('PC2', 'with variance', pca.var2[2])) +
  geom_label_repel() +
  labs(color = 'Condition') +
  theme_bw()+
  theme(legend.position = 'none')
...
ggsave(pca_22, file = paste0(figures, 'rnaseq_after_pca_plot.png'))
```
...
adjheatmap2
ggsave(adjheatmap2, file = paste0(figures, 'rnaseq_gene_significant_heatmap2.png'))
```
...
grid.newpage()
png(filename = paste0(figure, 'rnaseq_venn_diagram_rnaseq_up_down.png'))
