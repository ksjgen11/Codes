...
flagArrangeColumn = 0
Files = c("d:/reusf2/gene.norm.txt")
colorL=c("red","purple","blue","yellow","green","orange")
#for (ff in 1:length(Files)){
  #fname = Files[ff]
  E = read.delim(Files,row.names=1,header=T)
  
  mExp = E[,c("a_USF2_KO_MEF_G0_1","a_USF2_KO_MEF_G0_2","b_USF2_KO_MEF_G24_1","b_USF2_KO_MEF_G24_2",
              "c_WT_MEF_G0_1","c_WT_MEF_G0_2","d_WT_MEF_G24_1","d_WT_MEF_G24_2")]
  ...
  pngnamedist = paste("d:/reusf2/",Files,".dist.png",sep="")
  png(pngnamedist,width=500,height=500)
  par(cex=1.2)
  plot(h2,main="transcripts",cex.main=1)
  dev.off()
  
  pngname=paste("d:/reusf2/",Files,".Dendro.png",sep="") ; N.cluster=6; title="8"
  ...
  for(j in 1:N.cluster){
    ...
    clustername = paste("d:/reusf2/Cluster",fname,".",title,".",colorL[j],".",j,".txt",sep="")
    print(clustername)
  ...
  png("d:/reusf2/gene.norm.txt.Dendro2.png",width=1000,height=800)
  heatmap.2(as.matrix(my), Rowv=dend, Colv=F, scale="row",trace="none",dendrogram="row",labRow=F,
